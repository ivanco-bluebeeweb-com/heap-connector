"""Connection lifecycle for Heap Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from heap_client import HeapClient
...
@chat.function(
    "connect_heap",
    "Connect Heap account via credentials.",
    action_type="write",
    chain_callable=True,
    event="heap-connector.connect_heap",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_heap(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Heap Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "Heap Account",
...
@chat.function(
    "disconnect_heap",
    "Disconnect Heap account.",
    action_type="write",
    chain_callable=True,
    event="heap-connector.disconnect_heap",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_heap(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
